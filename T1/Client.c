...
void CopyFileFromDisk(struct Client* client, struct DiskScheduler * scheduler, const char* filename, bool sequential, int fileSize)
{
    ...
    int blockSize = scheduler->disk->blockSize;
    int blocks = (int) ceil ((double) fileSize / blockSize);
    if(blocks > scheduler->disk->blocks - 1)
    {
        printf("File is too big for the disk\n");
        exit(EXIT_FAILURE);
    ...
    {
        byte* buf = blocksArr[i] = (byte*) malloc(blockSize);
        struct Message *message = CreateMessage(ReadMessageType, client->buffer, blocksIdxs[i], i, buf);
    ...
    for(int i = 0; i < blocks; i++)
    {
        fwrite(blocksArr[i],blockSize, 1, file);
    }
    clock_gettime(CLOCK_MONOTONIC, &finish);
    double elapsed = (finish.tv_sec - start.tv_sec);
    elapsed += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
